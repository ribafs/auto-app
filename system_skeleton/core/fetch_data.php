...
if($nr > 0){
    ...
    while($row = $results->fetch(PDO::FETCH_ASSOC)) {
        ...
        $id = $row['id'];
            ...
            <td><a onclick="return confirm('Are you sure to want delete <?=$id?> ?')" href="delete.php?id=<?=$id?>&table=<?=$table?>"><i class="glyphicon glyphicon-remove-circle" title="Delete"></a></td></tr>
    <?php
    print "
        </tr>";
    }
